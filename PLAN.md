# SPLADE-mlx Project Plan

Problem: there is no SPLADE / V-SPLADE inference implementation specialized for Apple
...
| **P0** | `naver/efficient-splade-V-large-{query,doc}` | **DistilBERT** ×2 | **V-SPLADE**, asymmetric pair. (Confirmed during implementation: DistilBERT, not BERT) |
...
2. **Weight conversion** (`convert.py`)
   ...
   - V-SPLADE loads the query/doc checkpoints as one asymmetric pair API:
...
- Write **REPORT.md**:
  ...
  - highlight the asymmetric V-SPLADE query-encoding latency (on-device search angle)
